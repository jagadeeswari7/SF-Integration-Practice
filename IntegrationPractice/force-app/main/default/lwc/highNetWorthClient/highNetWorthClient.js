...
export default class HighNetWorthClient extends LightningElement {
    ...
    // Private properties
    displayMode;
    hasAccountRecordAccess = false;

    // Wire Apex method to fetch record access details
    @wire(determineRecordAccess, { recordId: '$recordId' })
    handleAccess({ error, data }) {
        if (data) {
            console.log("Record Access Data:", data);
            
            // Process record access details
            if (!data.hasRecordAccess) {
                this.hasAccountRecordAccess = false;
            } else {
                this.hasAccountRecordAccess = true;
                this.displayMode = data.mode;
            }
        } else if (error) {
            this.hasAccountRecordAccess = false;
            this.showToast('Error', 'Error checking record access. Please contact your administrator.', 'error');
        }
    }

    // Display toast message utility
